...
export async function* extractTypeRefs(
  ...
  for (const [file] of fileSymbols.entries()) {
    ...
    const edges = parseSourceWith(text, (root): TypeRefEdge[] => {
      ...
      for (const spec of structSpecs) {
        ...
        for (const fd of fieldDecls) {
          ...
          if (fieldType) {
            results.push({ edgeKind: "field_of_type", src: `${structName}.${fieldName}`, dst: fieldType, line: (fd.startPosition?.row ?? 0) + 1, metadata: { containment: "direct" } })

            const aggKey = `${structName}:${fieldType}`
            if (!emittedAggregates.has(aggKey)) {
              emittedAggregates.add(aggKey)
              results.push({ edgeKind: "aggregates", src: structName, dst: fieldType, line: (spec.startPosition?.row ?? 0) + 1 })
            }
          }
        }
      }
