...
export async function* extractCallbacks(
  ...
  for (const [file] of fileSymbols.entries()) {
    ...
    const registrations = parseSourceWith(text, (root): RawRegistration[] => {
      ...
      for (const callNode of callNodes) {
        ...
        for (const arg of argTexts) {
          ...
          if (knownFunctions.has(bare)) { callbackName = bare; break }
        }
        ...
        results.push({ kind: "call", registrar: calleeName, callbackName, registrationKind: "function_call", dispatchKey, line: callLine, file })
      }
      ...
      for (const pair of initPairs) {
        ...
        results.push({ kind: "init", registrar, callbackName: valueName, registrationKind: `struct_field:${containerType ?? "unknown"}.${fieldName}`, dispatchKey: fieldName, containerType: containerType ?? undefined, line: pairLine, file })
      }
      ...
      for (const fd of funcDefs) {
        ...
        if (decl) walkAst(decl, (n: any) => { if (!fnName && n.type === "identifier") fnName = n.text })
        if (fnName) {
          fnRanges.push({
            name: fnName,
      ...
      for (const assign of assignments) {
        ...
        results.push({ kind: "assign", registrar: enclosingFn, callbackName, registrationKind: `fn_body_assign:${containerExpr}.${fieldName}`, dispatchKey: fieldName, containerType: resolvedType, line: assignLine, file })
      }
    ...
    for (const reg of registrations) {
      if (reg.kind === "call") {
        yield* emitRegistration(reg.registrar, reg.callbackName, reg.registrationKind, reg.dispatchKey, reg.file, reg.line)
