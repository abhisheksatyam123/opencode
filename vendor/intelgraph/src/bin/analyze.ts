...
function parseArgs(argv: string[]): {
  ...
  for (const arg of args) {
    if (arg === "--json") {
      ...
      else { console.error("--compare= requires a positive integer snapshot ID"); process.exit(2) }
...
function findDbPath(workspace: string): string | null {
  const paths = [
    join(workspace, ".intelgraph", "intelligence.db"),
    join(workspace, ".intelgraph", "intelligence.db"),
  ]
...
async function main() {
  ...
  try {
    ...
    if (actionPlan) {
      ...
      if (format === "json") {
        ...
        for (const r of planResult.rows) {
          ...
          lines.push(`| ${row.rank} | ${row.priority} | ${row.category} | ${row.action} | \`${String(row.target).split("#").pop()}\` | ${row.detail} |`)
        }
    ...
    if (compareWith !== null) {
      ...
      if (format === "json") {
        ...
        for (const r of diffResult.rows) {
          ...
          lines.push(`| ${row.metric} | ${row.previous ?? "—"} | ${row.current} | ${row.delta != null ? (Number(row.delta) > 0 ? `+${row.delta}` : String(row.delta)) : "—"} | ${trendIcon[String(row.trend)] ?? "?"} |`)
        }
        if (moduleDiff.rows.length > 0) {
          ...
          for (const r of moduleDiff.rows) {
            ...
            const delta = row.delta_symbols != null ? (Number(row.delta_symbols) > 0 ? `+${row.delta_symbols}` : String(row.delta_symbols)) : "—"
            lines.push(`| ${changeIcon[String(row.change)] ?? ""} ${row.change} | ${delta} | ${row.current_symbols ?? "—"} | ${row.prev_symbols ?? "—"} | \`${row.canonical_name}\` |`)
          }
        }
        console.log(lines.join("\n"))
      } else {
        ...
        for (const r of diffResult.rows) {
          ...
          console.log(`  ${(String(row.metric)).padEnd(20)} ${String(row.previous ?? "—").padStart(8)} → ${String(row.current).padStart(8)}${delta}  ${trendIcon[String(row.trend)] ?? ""}`)
        }
        if (moduleDiff.rows.length > 0) {
          ...
          for (const r of moduleDiff.rows) {
            const row = r as Record<string, unknown>
            const changeIcon: Record<string, string> = { added: "+", removed: "-", grown: "↑", shrunk: "↓" }
            const delta = row.delta_symbols != null ? ` Δ${Number(row.delta_symbols) > 0 ? "+" : ""}${row.delta_symbols}` : ""
            console.log(`  ${(changeIcon[String(row.change)] ?? " ").padEnd(2)} [${String(row.change).padEnd(9)}]${delta.padEnd(8)}  ${row.canonical_name}`)
          }
        }
        console.log(sep)
      }
    ...
    const [health, problematic, godClasses, typeHealth, deadCode, refactors, healthReport] =
      await Promise.all([
        wantHealth ? lookup.lookup({ intent: "find_workspace_health", snapshotId }) : Promise.resolve({ rows: [] as Array<Record<string, unknown>> }),
        wantMods   ? lookup.lookup({ intent: "analyze_problematic_modules", snapshotId, limit }) : Promise.resolve({ rows: [] as Array<Record<string, unknown>> }),
        wantGod    ? lookup.lookup({ intent: "analyze_god_classes", snapshotId, limit }) : Promise.resolve({ rows: [] as Array<Record<string, unknown>> }),
        wantGod    ? lookup.lookup({ intent: "analyze_type_health", snapshotId, limit }) : Promise.resolve({ rows: [] as Array<Record<string, unknown>> }),
        wantDead   ? lookup.lookup({ intent: "analyze_dead_code", snapshotId, limit }) : Promise.resolve({ rows: [] as Array<Record<string, unknown>> }),
        wantRef    ? lookup.lookup({ intent: "suggest_refactors", snapshotId, limit }) : Promise.resolve({ rows: [] as Array<Record<string, unknown>> }),
        wantHealth ? lookup.lookup({ intent: "generate_health_report", snapshotId, limit: 15 }) : Promise.resolve({ rows: [] as Array<Record<string, unknown>> }),
      ])
    ...
    if (format === "markdown") {
      ...
      if (problematic.rows.length > 0) {
        ...
        lines.push(mdTable(
          ...
          problematic.rows.map((r) => [
            str(r.problem_score), num(r.dead_exports), num(r.outgoing_imports), `\`${str(r.canonical_name)}\``,
          ]),
        ))
        lines.push("")
      }
      ...
      if (godClasses.rows.length > 0) {
        ...
        lines.push(mdTable(
          ...
          godClasses.rows.map((r) => [
            str(r.complexity_score), num(r.method_count), num(r.field_count),
            str(r.recommendation), `\`${str(r.canonical_name)}\``,
          ]),
        ))
        lines.push("")
      }
      ...
      if (deadCode.rows.length > 0) {
        ...
        lines.push(mdTable(
          ...
          deadCode.rows.map((r) => [
            str(r.kind), str(r.dead_reason), str(r.action), `\`${str(r.canonical_name)}\``,
          ]),
        ))
        lines.push("")
      }
      ...
      if (refactors.rows.length > 0) {
        ...
        lines.push(mdTable(
          ["edges", "source", "→", "target"],
          refactors.rows.map((r) => [
            num(r.coupling_count), `\`${str(r.source)}\``, "→", `\`${str(r.target)}\``,
          ]),
        ))
        lines.push("")
      }
    ...
    if (health.rows[0]) {
      ...
      console.log(`  Modules: ${h.modules_count}  Classes: ${h.classes_count}  Types: ${h.types_count}  APIs: ${h.apis_count}`)
    ...
    if (problematic.rows.length > 0) {
      console.log(textSection(
        "Problematic modules",
        problematic.rows,
        ["canonical_name", "problem_score", "dead_exports", "outgoing_imports"],
      ))
    }
    ...
    if (godClasses.rows.length > 0) {
      console.log(textSection(
        ...
        godClasses.rows,
        ["canonical_name", "complexity_score", "method_count", "field_count", "recommendation"],
      ))
    }

    if (typeHealth.rows.length > 0) {
      const badTypes = typeHealth.rows.filter(
        (r) => (r as { health_status?: string }).health_status !== "healthy",
      )
      if (badTypes.length > 0) {
        console.log(textSection(
          "Type health issues (unused / hotspot)",
          badTypes,
          ["canonical_name", "health_status", "consumers", "field_touches"],
        ))
      }
    }
    ...
    if (deadCode.rows.length > 0) {
      console.log(textSection(
        "Dead code",
        deadCode.rows,
        ["canonical_name", "kind", "dead_reason", "action"],
      ))
    }
    ...
    if (refactors.rows.length > 0) {
      console.log(textSection(
        ...
        refactors.rows,
        ["source", "target", "coupling_count"],
      ))
    }

    if (healthReport.rows.length > 0) {
      console.log(textSection(
        "Module health report (worst first)",
        healthReport.rows,
        ["canonical_name", "module_health_score", "api_count", "dead_api_count", "outgoing_imports"],
      ))
    }
...
async function entryPoint() {
  ...
  let lastMtime = 0
  try { lastMtime = statSync(watchDbPath).mtimeMs } catch { /* ignore */ }
  ...
  const interval = setInterval(async () => {
    ...
    } catch { /* DB locked mid-write — retry next tick */ }
  }, 750)
