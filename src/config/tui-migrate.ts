...
import { TuiInfo, TuiOptions } from "@/config/tui-schema"
...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.OPENCODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("opencode", InstanceContextStorage.directory, InstanceContextStorage.worktree)
  const files = [...project]
  const globalConfig = path.resolve(Global.Path.config)
  for (const dir of unique(input.directories)) {
    const resolved = path.resolve(dir)
    const relative = path.relative(globalConfig, resolved)
    if (relative === "" || (!relative.startsWith("..") && !path.isAbsolute(relative))) continue
    files.push(...ConfigPaths.fileInDirectory(resolved, "opencode"))
  }
