...
} from "@/provider/sdk/copilot/chat/openai-compatible-chat-options"
import {
  defaultOpenAICompatibleErrorStructure,
  type ProviderErrorStructure,
} from "@/provider/sdk/copilot/openai-compatible-error"
import type { MetadataExtractor } from "@/provider/sdk/copilot/chat/openai-compatible-metadata-extractor"
...
export class OpenAICompatibleChatLanguageModel implements LanguageModelV3 {
  ...
  private get providerOptionsName(): string {
    return this.config.provider.split("@/provider/sdk/copilot/chat")[0].trim()
  }
    ...
    return {
      args: {
        ...
        messages: convertToOpenAICompatibleChatMessages(prompt, {
          // Vertex AI requires `thought_signature` on every replayed tool-call
          // + tool-result block for ALL Gemini models (not just Gemini 3).
          // The session-layer normally injects a real or sentinel value, but
          // tool-calls rewritten by `experimental_repairToolCall`
          // (e.g. `default_api:read` → `read`, `default_api:bash` → `bash`)
          // and other edge cases can drop the metadata before reaching the wire.
          // Provide the documented skip sentinel as last-resort fallback when
          // the route id is Gemini-on-Vertex
          // or a bare Gemini model id that requires thought signatures.
          defaultThoughtSignature: isGemini3LikeId(this.modelId) ? SKIP_THOUGHT_SIGNATURE : undefined,
        }),
  ...
  async doGenerate(options: LanguageModelV3CallOptions) {
    ...
    if (choice.message.tool_calls != null) {
      for (const toolCall of choice.message.tool_calls) {
        const toolThoughtSignature = toolCall.extra_content?.google?.thought_signature ?? undefined
        const reasoningOpaqueMeta = choice.message.reasoning_opaque
          ? { copilot: { reasoningOpaque: choice.message.reasoning_opaque } }
          : {}
        const thoughtSignatureMeta = toolThoughtSignature
          ? { [this.providerOptionsName]: { thoughtSignature: toolThoughtSignature } }
          : {}
        const mergedMeta = { ...reasoningOpaqueMeta, ...thoughtSignatureMeta } as SharedV3ProviderMetadata
        content.push({
          type: "tool-call",
          toolCallId: toolCall.id ?? generateId(),
          toolName: toolCall.function.name,
          input: toolCall.function.arguments!,
          ...(Object.keys(mergedMeta).length > 0 ? { providerMetadata: mergedMeta } : {}),
        })
      }
    }
    ...
    return {
      stream: response.pipeThrough(
        new TransformStream<ParseResult<z.infer<typeof this.chunkSchema>>, LanguageModelV3StreamPart>({
          ...
          // ARCH-DEBT: Chunk type safety lost due to error schema union in the stream parser.
          // The `chunk` here is `{ success: true; value: T } | { success: false; error: ZodError }`
          // but the transform callback receives `unknown` because the error branch widens the type.
          // Fix: narrow the union explicitly or use a typed discriminated union for the stream chunks.
              ...
              for (const toolCallDelta of delta.tool_calls) {
                    ...
                    if (isParsableJson(toolCall.function.arguments)) {
                      ...
                      const _fastMeta1 = {
                        ...
                        ...(toolCall.thoughtSignature
                          ? { [providerOptionsName]: { thoughtSignature: toolCall.thoughtSignature } }
                          : {}),
                      }
                      controller.enqueue({
                        type: "tool-call",
                        ...
                        toolName: toolCall.function.name,
                        input: toolCall.function.arguments,
                        ...(Object.keys(_fastMeta1).length > 0 ? { providerMetadata: _fastMeta1 } : {}),
                      })
                ...
                if (
                  ...
                  controller.enqueue({
                    ...
                    toolName: toolCall.function.name,
                    input: toolCall.function.arguments,
                    ...(Object.keys(_deltaMeta2).length > 0 ? { providerMetadata: _deltaMeta2 } : {}),
                  })
                  toolCall.hasFinished = true
                }
              }
          ...
          flush(controller) {
            ...
            for (const toolCall of toolCalls.filter((toolCall) => !toolCall.hasFinished)) {
              ...
              controller.enqueue({
                ...
                toolName: toolCall.function.name,
                input: toolCall.function.arguments,
                ...(Object.keys(_flushMeta3).length > 0 ? { providerMetadata: _flushMeta3 } : {}),
              })
            }
