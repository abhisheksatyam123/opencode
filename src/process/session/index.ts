...
import { ProjectTable } from "@/config/project/project.sql"
import { Storage } from "@/storage/contract/port"
import { StorageDefaultLayer } from "@/storage/layer"
import { Log } from "@/foundation/util/log"
import { updateSchema } from "@/foundation/util/update-schema"
...
import {
  buildPacket,
  hasActionableStateChanged,
  markPacketStale,
  nextActionable,
} from "@/process/session/context-packet"
...
import { ProjectID } from "@/config/project/schema"
...
export namespace Session {
  ...
  const sessionMemoryStore = new Map<SessionID, Record<string, string>>()
  ...
  export function toRow(info: Info) {
    return {
      ...
      permission_mode: info.permissionMode ?? "default",
  ...
  export const Info = z
    .object({
      ...
      permission: z.lazy(() => Permission.Ruleset).optional(),
      permissionMode: z.lazy(() => Permission.Mode).optional(),
      revert: z
        .object({
  ...
  export const getUsage = (input: {
    ...
    const cacheWriteInputTokens = safe(
      (input.metadata?.["anthropic"]?.["cacheCreationInputTokens"] ??
        ...
        // @ts-expect-error — bedrock usage shape not in AI SDK types; runtime field access is safe
        input.metadata?.["bedrock"]?.["usage"]?.["cacheWriteInputTokens"] ??
        // @ts-expect-error — venice usage shape not in AI SDK types; runtime field access is safe
    ...
    const adjustedInputTokens = Math.max(0, safe(inputTokens - cacheReadInputTokens - cacheWriteInputTokens))
    ...
    Effect.gen(function* () {
      ...
      const share = Effect.fn("Session.share")(function* (id: SessionID) {
        ...
        const result = yield* Effect.promise(async () => {
          const { ShareNext } = await import("@/surface/share/share-next")
      ...
      const diff = Effect.fn("Session.diff")(function* (sessionID: SessionID) {
        return yield* Storage.Service.use((storage) =>
          storage.read<Snapshot.FileDiff[]>(["session_diff", sessionID]),
        ).pipe(
          Effect.orElseSucceed(() => [] as Snapshot.FileDiff[]),
          Effect.provide(StorageDefaultLayer),
        )
      })
  ...
  type SessionMemorySetInput = { sessionID: SessionID; key: string; value: string }
  type SessionMemoryGetInput = { sessionID: SessionID; key: string }
  type SessionMemoryListInput = { sessionID: SessionID }

  function normalizeSetInput(
    inputOrSessionID: SessionMemorySetInput | SessionID,
    key?: string,
    value?: string,
  ): SessionMemorySetInput {
    if (typeof inputOrSessionID === "object") return inputOrSessionID
    if (!key || value === undefined) throw new Error("setSessionMemory requires sessionID, key, value")
    return { sessionID: inputOrSessionID, key, value }
  }

  function normalizeGetInput(inputOrSessionID: SessionMemoryGetInput | SessionID, key?: string): SessionMemoryGetInput {
    if (typeof inputOrSessionID === "object") return inputOrSessionID
    if (!key) throw new Error("getSessionMemory requires sessionID and key")
    return { sessionID: inputOrSessionID, key }
  }

  function normalizeListInput(inputOrSessionID: SessionMemoryListInput | SessionID): SessionMemoryListInput {
    if (typeof inputOrSessionID === "object") return inputOrSessionID
    return { sessionID: inputOrSessionID }
  }
  ...
  export async function setSessionMemory(
    ...
    const input = normalizeSetInput(inputOrSessionID, key, value)
    const current = sessionMemoryStore.get(input.sessionID) ?? {}
    current[input.key] = input.value
    sessionMemoryStore.set(input.sessionID, current)
    return { key: input.key, value: input.value }
  }

  export async function getSessionMemory(inputOrSessionID: SessionMemoryGetInput | SessionID, key?: string) {
    const input = normalizeGetInput(inputOrSessionID, key)
    const memory = sessionMemoryStore.get(input.sessionID)
    return memory?.[input.key]
  }

  export async function listSessionMemory(inputOrSessionID: SessionMemoryListInput | SessionID) {
    const input = normalizeListInput(inputOrSessionID)
    const memory = sessionMemoryStore.get(input.sessionID)
    return memory ? { ...memory } : {}
  }

  export async function clearSessionMemory(inputOrSessionID: SessionMemoryListInput | SessionID) {
    const input = normalizeListInput(inputOrSessionID)
    sessionMemoryStore.delete(input.sessionID)
  }
  ...
  export const setPermission = fn(
    z.object({ sessionID: SessionID.zod, permission: z.lazy(() => Permission.Ruleset) }),
...
ToolSessionPort.registerBridge({
  sessionCreate: (input) => Session.create(input),
  sessionGet: (id) => Session.get(id),
  updatePart: (part) => Session.updatePart(part),
  messageGet: (input) => MessageV2.get(input),
  messagePage: (input) => MessageV2.page(input),
  prompt: (input) => SessionPrompt.prompt(input),
  resolvePromptParts: (parts) => SessionPrompt.resolvePromptParts(parts),
  onCancel: (sessionID, fn) => SessionPrompt.onCancel(sessionID, fn),
  cancel: (sessionID) => SessionPrompt.cancel(sessionID),
  buildPacket: (input) => buildPacket(input),
  nextActionable: (planText, taskPath) => nextActionable(planText, taskPath),
  hasActionableStateChanged: (prev, next) => hasActionableStateChanged(prev, next),
  markPacketStale: (sessionID, cause, force) => markPacketStale(sessionID, cause, force),
})
