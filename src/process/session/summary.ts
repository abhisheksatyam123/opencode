...
import { SessionID, MessageID } from "@/process/session/schema"
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const summarize = Effect.fn("SessionSummary.summarize")(function* (input: {
        ...
        // Lifecycle hook — fires alongside Session.Event.Diff so plugins
        // can react to detected file changes (IDE refresh triggers, external
        // file watchers, audit logs, etc.). Only path is included by default;
        // before/after content is omitted to keep the payload small. Plugins
        // that need content can call back into the storage layer.
        yield* Effect.promise(() =>
          ProviderPluginHooks.notify("file.changed", {
            sessionID: input.sessionID,
            diffs: diffs.map((d) => ({ path: d.file })),
          }),
        ).pipe(Effect.ignore)
  ...
  export const defaultLayer = Layer.unwrap(
    Effect.sync(() =>
      layer.pipe(
        ...
        Layer.provide(StorageDefaultLayer),
