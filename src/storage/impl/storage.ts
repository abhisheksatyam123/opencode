...
import { StorageService } from "@/storage/contract/port"
import { Effect, Exit, Layer, Option, RcMap, Schema, TxReentrantLock } from "effect"
...
export namespace Storage {
  ...
  export const Service = StorageService
  export type Service = StorageService
      ...
      for (const projectDir of projectDirs) {
        ...
        if (projectID !== "global") {
          ...
          const result = yield* Effect.promise(() =>
            Process.text(["git", "rev-list", "--max-parents=0", "--all"], {
              cwd: worktree,
              nothrow: true,
            }),
          )
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const state = yield* Effect.cached(
        Effect.gen(function* () {
          const dir = vaultPath.state("data", "storage")
