...
import { SessionTable, MessageTable, PartTable, TodoTable, PermissionTable } from "@/storage/session.sql"
import { SessionShareTable } from "@/storage/share.sql"
...
export namespace JsonMigration {
  ...
  type MigrationValue = Record<string, unknown>

  export async function run(sqlite: Database, options?: Options) {
    const storageDir = vaultPath.state("data", "storage")
    ...
    // Drizzle's insert type is table-specific, but this migration batches
    // heterogeneous legacy JSON rows through one helper. Keep the table boundary
    // open and type the row payloads as generic migration records.
    function insert(values: MigrationValue[], table: any, label: string) {
    ...
    async function migrateProjects() {
      ...
      const projectValues = [] as MigrationValue[]
    ...
    async function migrateSessions() {
      ...
      const sessionValues = [] as MigrationValue[]
    ...
    async function migrateTodos() {
      const values = [] as MigrationValue[]
    ...
    async function migratePermissions() {
      const permValues = [] as MigrationValue[]
    ...
    async function migrateShares() {
      const shareValues = [] as MigrationValue[]
