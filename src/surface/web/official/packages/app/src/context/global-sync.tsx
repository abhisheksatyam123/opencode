...
export const loadMcpQuery = (directory: string, sdk: OpencodeClient) =>
  queryOptions({
    ...
    queryFn: () => {
      const status = (sdk as { mcp?: { status?: () => Promise<{ data?: unknown }> } }).mcp?.status
      if (!status) return Promise.resolve({})
      return status().then((r) => r.data ?? {})
    },
  })
...
export const loadLspQuery = (directory: string, sdk: OpencodeClient) =>
  queryOptions({
    ...
    queryFn: () => {
      const status = (sdk as { lsp?: { status?: () => Promise<{ data?: unknown }> } }).lsp?.status
      if (!status) return Promise.resolve([])
      return status().then((r) => r.data ?? [])
    },
  })
...
function createGlobalSync() {
  ...
  async function loadSessions(directory: string) {
    ...
    if (meta && meta.limit >= store.limit) {
      const next = trimSessions(store.session, { limit: store.limit })
        ...
        queryFn: () =>
          loadRootSessionsWithFallback({
            ...
            .then((x) => {
              ...
              const sessions = trimSessions([...nonArchived, ...childSessions], { limit })
              batch(() => {
                setStore(
                  "sessionTotal",
