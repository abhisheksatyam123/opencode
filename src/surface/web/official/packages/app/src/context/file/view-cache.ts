...
function createViewSession(dir: string, id: string | undefined) {
  ...
  const selectedCharacter = (path: string) => view.file[path]?.selectedCharacter
  ...
  const setSelectedCharacter = (path: string, character: number | null) => {
    setView(
      produce((draft) => {
        const file = draft.file[path] ?? (draft.file[path] = {})
        if ((file.selectedCharacter ?? null) === character) return
        file.selectedCharacter = character
      }),
    )
    pruneView(path)
  }
