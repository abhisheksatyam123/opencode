  ...
  init: () => {
    ...
    const evictContent = (keep?: Set<string>) => {
      evictContentLru(keep, (target) => {
        if (!store.file[target] || store.file[target]?.dirty) return
        setStore(
          "file",
          target,
          produce((draft) => {
    ...
    const setLoaded = (file: string, content: FileState["content"]) => {
      setStore(
        ...
        produce((draft) => {
          ...
          draft.error = undefined
          if (!draft.dirty) {
            draft.editBuffer = content?.content ?? ""
            draft.dirty = false
    ...
    const updateBuffer = (input: string, content: string) => {
      const file = path.normalize(input)
      if (!file) return
      ensure(file)
      setStore(
        ...
        file,
        produce((draft) => {
          draft.editBuffer = content
          draft.dirty = content !== (draft.content?.content ?? "")
          draft.saveError = undefined
        }),
      )
    }
    ...
    const save = async (input: string) => {
      ...
      const current = store.file[file]
      const content = current?.editBuffer ?? current?.content?.content ?? ""
      setStore(
        "file",
        file,
        produce((draft) => {
        ...
        setStore(
          ...
          produce((draft) => {
            ...
            draft.content = {
              type: "text",
              content,
              mimeType: draft.content?.mimeType,
            }
          }),
        )
        touchFileContent(file, approxBytes({ type: "text", content }))
        evictContent(new Set([file]))
      } catch (e) {
    ...
    const selectedCharacter = (input: string) => withPath(input, (file) => view().selectedCharacter(file))
    ...
    const setSelectedCharacter = (input: string, character: number | null) =>
      withPath(input, (file) => view().setSelectedCharacter(file, character))
