...
export namespace ServerConnection {
  type Base = { displayName?: string; authRole?: "read" | "write" }
...
export const { use: useServer, provider: ServerProvider } = createSimpleContext({
  ...
  init: (props: {
    ...
    const [state, setState] = createStore({
      ...
      authRole: undefined as "read" | "write" | undefined,
    })

    const healthy = () => state.healthy
    const authRole = () => state.authRole ?? current()?.authRole ?? "write"
    ...
    function startHealthPolling(conn: ServerConnection.Any) {
      ...
      const run = () => {
        ...
        void check(conn)
          .then((next) => {
            if (!alive) return
            setState({ healthy: next.healthy, authRole: next.authRole })
          })
          .finally(() => {
            busy = false
          })
    ...
    createEffect(() => {
      ...
      if (props.disableHealthCheck) {
        ...
        void check(current_).then((next) => setState({ authRole: next.authRole }))
        return
      }
      setState({ healthy: undefined, authRole: undefined })
