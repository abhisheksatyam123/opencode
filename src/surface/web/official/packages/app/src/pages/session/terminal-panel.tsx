...
export function TerminalPanel() {
  ...
  const opened = createMemo(() => view().terminal.opened() && !readOnly())
  ...
  createEffect(() => {
    if (!opened() || !terminal.ready()) return
    if (terminal.active()) return
    const first = all()[0]?.id
    if (first) terminal.open(first)
  })
            ...
            <div class="flex flex-col h-full">
              ...
              <div class="flex-1 min-h-0 relative">
                <Switch>
                    ...
                    <div class="absolute inset-0 flex items-center justify-center bg-background-base px-6 text-center">
                      <div class="flex max-w-sm flex-col items-center gap-3">
                        <div class="text-14-medium text-text-strong">
                          {store.autoCreated ? "Starting terminal session…" : "No terminal session"}
                        </div>
                        <div class="text-12-regular text-text-weak">
                          {store.autoCreated
                            ? "Waiting for the server to create and attach a PTY."
                            : "Open a workspace shell in this panel."}
                        </div>
                        <button
                          type="button"
                          class="rounded-md border border-border-weaker-base px-3 py-1.5 text-12-medium text-text-base hover:border-accent hover:text-accent"
                          onClick={terminal.new}
                        >
                  ...
                  <Match when={terminal.active()} keyed>
                    {(id) => {
