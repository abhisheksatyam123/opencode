...
const WorkspaceActions = (props: {
  ...
  newSessionActive: Accessor<boolean>
}): JSX.Element => (
  <div
  ...
  >
    ...
    <Show when={!props.touch()}>
      <Tooltip value={props.language.t("command.session.new")} placement="top">
        <IconButton
          icon={props.newSessionActive() ? "new-session-active" : "new-session"}
          variant="ghost"
          class="size-6 rounded-md"
          classList={{
            "bg-surface-raised-base-active opacity-100 pointer-events-auto": props.newSessionActive(),
            "opacity-0 pointer-events-none group-hover/workspace:opacity-100 group-hover/workspace:pointer-events-auto group-focus-within/workspace:opacity-100 group-focus-within/workspace:pointer-events-auto":
              !props.newSessionActive(),
          }}
          data-action="workspace-new-session"
          data-workspace={base64Encode(props.directory)}
          aria-label={props.language.t("command.session.new")}
          aria-current={props.newSessionActive() ? "page" : undefined}
          onClick={(event) => {
            event.preventDefault()
...
export const SortableWorkspace = (props: {
  ...
  const creating = createMemo(() => active() && !params.id)
  ...
  const touch = breakpoints.isTouch
