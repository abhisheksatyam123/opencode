...
function isOpenableProjectDirectory(directory: string | undefined) {
  return !!directory && directory !== "/"
}

const AUTOSELECT_TIMEOUT_MS = 2_500
const SESSION_RESTORE_TIMEOUT_MS = 2_000

type StoredSessionRoute = { directory: string; id: string; at: number }

function withTimeout<TValue>(promise: Promise<TValue>, ms: number, fallback: TValue) {
  return Promise.race([
    promise,
...
export default function Layout(props: ParentProps) {
  const [store, setStore, , ready] = persisted(
    ...
    createStore({
      lastProjectSession: {} as Record<string, StoredSessionRoute>,
      lastWorkspaceSession: {} as Record<string, StoredSessionRoute>,
  ...
  const [autoselecting] = createResource(async () => {
    const readyResult = await withTimeout(
      Promise.all([ready.promise, layout.ready.promise]).then(() => true),
      AUTOSELECT_TIMEOUT_MS,
      false,
    )
    if (!readyResult) {
      logProjectOpen("warn", "autoselect.timeout_waiting_for_persistence", { timeoutMs: AUTOSELECT_TIMEOUT_MS })
      return
    }
    if (!untrack(() => state.autoselect)) return

    const list = layout.projects.list().filter((project) => isOpenableProjectDirectory(project.worktree))
    const last = server.projects.last()

    if (list.length === 0) {
      if (!last || !isOpenableProjectDirectory(last)) return
  ...
  async function archiveSession(session: Session) {
    ...
    clearArchivedSessionReferences(session, nextSession)
  ...
  function rememberWorkspaceSessionRoute(directory: string, id: string) {
    setStore("lastWorkspaceSession", directory, { directory, id, at: Date.now() })
  }
  ...
  function clearLastWorkspaceSession(directory: string) {
    if (!store.lastWorkspaceSession[directory]) return
    setStore(
  ...
  function clearArchivedSessionReferences(session: Pick<Session, "directory" | "id">, replacement?: Session) {
    for (const [root, route] of Object.entries(store.lastProjectSession)) {
      if (route.id !== session.id) continue
      if (replacement) rememberSessionRoute(replacement.directory, replacement.id, root)
      else clearLastProjectSession(root)
    }

    const workspaceRoute = store.lastWorkspaceSession[session.directory]
    if (workspaceRoute?.id !== session.id) return
    if (replacement?.directory === session.directory)
      rememberWorkspaceSessionRoute(replacement.directory, replacement.id)
    else clearLastWorkspaceSession(session.directory)
  }

  function syncSessionRoute(directory: string, id: string, root = activeProjectRoot(directory)) {
    rememberSessionRoute(directory, id, root)
    rememberWorkspaceSessionRoute(directory, id)
  ...
  async function navigateToProject(directory: string | undefined) {
    if (!directory || !isOpenableProjectDirectory(directory)) {
      logProjectOpen("warn", "navigate_to_project.skipped_unopenable_directory", { directory })
      return
    }
    logProjectOpen("info", "navigate_to_project.start", { directory })
    ...
    let dirs = project
      ...
      : [root]
    logProjectOpen("debug", "navigate_to_project.workspace_candidates", { root, directories: dirs })
    ...
    const refreshDirs = async (target?: string) => {
      ...
      const listed = await globalSDK.client.worktree
        ...
        .then((x) => x.data ?? [])
        .catch((error) => {
          logProjectOpen("warn", "navigate_to_project.worktree_list_failed", {
            root,
            target,
      ...
      logProjectOpen("debug", "navigate_to_project.worktree_list_refreshed", { root, directories: dirs })
      return canOpen(target)
    }
    const seedSession = (directory: string, info: Session) => {
    ...
    const openSession = async (target: { directory: string; id: string }) => {
      if (!canOpen(target.directory)) {
        logProjectOpen("debug", "navigate_to_project.session_rejected_directory", {
          root,
          target,
          directories: dirs,
        })
      ...
      if (data.session.some((item) => item.id === target.id)) {
        ...
        rememberWorkspaceSessionRoute(target.directory, target.id)
        logProjectOpen("info", "navigate_to_project.session_opened_from_cache", {
          root,
      ...
      const resolved = await withTimeout(
        globalSDK.client.session
          ...
          .then((x) => x.data)
          .catch((error) => {
            logProjectOpen("warn", "navigate_to_project.session_lookup_failed", {
              root,
              target,
              error: diagnosticError(error),
            })
      ...
      if (!resolved?.directory) {
        logProjectOpen("debug", "navigate_to_project.session_lookup_empty", { root, target })
        return false
      }
      if (!canOpen(resolved.directory)) {
        logProjectOpen("warn", "navigate_to_project.session_lookup_outside_workspace", {
          root,
          target,
      ...
      rememberWorkspaceSessionRoute(resolved.directory, resolved.id)
      logProjectOpen("info", "navigate_to_project.session_opened_from_lookup", {
        root,
    ...
    const openLastProjectSession = async () => {
      const projectSession = store.lastProjectSession[root]
      if (!projectSession?.id) return false
    ...
    const targetIsProjectRoot = pathKey(directory) === pathKey(root)

    const workspaceSession = store.lastWorkspaceSession[directory]
    if (workspaceSession?.id) {
      const opened = await openSession(workspaceSession)
      if (opened) return
      clearLastWorkspaceSession(directory)
    }

    if (targetIsProjectRoot && (await openLastProjectSession())) return

    const latestForDirectory = latestRootSession(
      [{ path: { directory }, session: globalSync.child(directory, { bootstrap: false })[0].session }],
      Date.now(),
    )
    if (latestForDirectory && (await openSession(latestForDirectory))) {
      return
    }

    const fetchedForDirectory = latestRootSession(
      [
        {
          path: { directory },
          session: await withTimeout(
            globalSDK.client.session
              ...
              .then((x) => x.data ?? [])
              .catch((error) => {
                logProjectOpen("warn", "navigate_to_project.workspace_session_list_failed", {
                  root,
                  directory,
                  error: diagnosticError(error),
                })
    ...
    if (fetchedForDirectory && (await openSession(fetchedForDirectory))) {
      return
    }
        ...
        dirs.map(async (item) => ({
          ...
          session: await withTimeout(
            globalSDK.client.session
              ...
              .then((x) => x.data ?? [])
              .catch((error) => {
                logProjectOpen("warn", "navigate_to_project.session_list_failed", {
                  root,
                  directory: item,
                  error: diagnosticError(error),
                })
    ...
    const created = await globalSDK.client.session
      ...
      .then((x) => x.data)
      .catch((error) => {
        logProjectOpen("warn", "navigate_to_project.create_session_failed", {
          root,
          error: diagnosticError(error),
        })
        return undefined
    ...
    if (created?.id) {
      const directory = created.directory || root
      seedSession(directory, created)
      setStore("lastProjectSession", root, { directory, id: created.id, at: Date.now() })
      layout.handoff.setTabs(base64Encode(directory), created.id)
      logProjectOpen("info", "navigate_to_project.session_created_for_empty_project", {
        root,
        directory,
        sessionID: created.id,
      })
      navigateWithSidebarReset(`/${base64Encode(directory)}/session/${created.id}`)
      return
    }

    logProjectOpen("info", "navigate_to_project.fallback_to_project_session", { root })
  ...
  function openProject(directory: string, navigate = true) {
    if (!isOpenableProjectDirectory(directory)) {
      logProjectOpen("warn", "open_project.skipped_unopenable_directory", { directory, navigate })
      return
    }

    logProjectOpen("info", "open_project.request", { directory, navigate })
    try {
      layout.projects.open(directory)
      logProjectOpen("debug", "open_project.added_to_sidebar", { directory })
    } catch (error) {
      logProjectOpen("error", "open_project.add_failed", {
        directory,
        error: diagnosticError(error),
      })
      throw error
    ...
    if (!navigate) return
    const task = navigateToProject(directory)
    void task
      .then(() => {
        logProjectOpen("info", "open_project.navigate_completed", { directory })
      })
      .catch((error) => {
        logProjectOpen("error", "open_project.navigate_failed", {
          directory,
          error: diagnosticError(error),
        })
      })
          ...
          <div class="size-full relative overflow-x-hidden">
            ...
            <div class="md:hidden">
              <div
                ...
                style={{
                  top: "calc(2.5rem + var(--safe-area-inset-top))",
            ...
            >
              <main
                classList={{
                  "size-full overflow-x-hidden flex flex-col items-start contain-strict bg-background-base md:border-l md:rounded-tl-[12px]": true,
                }}
              >
