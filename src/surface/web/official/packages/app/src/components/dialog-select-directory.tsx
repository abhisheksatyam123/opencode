...
function displayPath(path: string, input: string, home: string) {
  ...
  if (modeOf(input) === "tilde") return tildeOf(full, home) || full
  return full
}
...
function useDirectorySearch(args: {
  ...
  const logSearchError = (message: string, extra: Record<string, unknown>) => {
    const dedupe = `${message}:${JSON.stringify(extra)}`
    if (seenErrors.has(dedupe)) return
    seenErrors.add(dedupe)
    emitDiagnosticLog({
      client: args.sdk.client,
      service: "web.project-picker",
      level: "warn",
      message,
      extra,
    })
  ...
  const dirs = async (dir: string) => {
    ...
    const request = args.sdk.client.file
      ...
      .then((x) => x.data ?? [])
      .catch((error) => {
        logSearchError("directory_list_failed", {
          directory: key,
  ...
  return async (filter: string) => {
    ...
    const find = () =>
      args.sdk.client.find
        ...
        .then((x) => x.data ?? [])
        .catch((error) => {
          logSearchError("directory_find_failed", {
