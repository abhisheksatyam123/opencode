  ...
  onMount(() => {
    const run = async () => {
      ...
      const connectToken = async () => {
        const connectTokenRequest = (
          client.pty as {
            connectToken?: (
              params: { ptyID: string; directory: string },
              options: { throwOnError: false; headers: Record<string, string> },
            ) => Promise<{ response: Response; data?: unknown }>
          }
        ).connectToken
        if (!connectTokenRequest) return

        const result = await connectTokenRequest
          .call(
            client.pty,
            { ptyID: id, directory },
            {
          ...
          .catch((err: unknown) => {
            ...
            debugTerminal("connect-token request failed; continuing without ticket", err)
            return
          })
        if (!result) return
        ...
        if (status === 200) {
          ...
          debugTerminal("connect-token response missing ticket; continuing without ticket", {
            status,
        ...
        debugTerminal("connect-token unsupported status; continuing without ticket", { status })
      }
