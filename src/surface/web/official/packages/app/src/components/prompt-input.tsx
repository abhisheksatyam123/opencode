...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const sessionID = createMemo(() => props.sessionIDOverride ?? params.id)
  const promptScope = createMemo(() => {
    const id = sessionID()
    if (!id) return undefined
    return { dir: sdk.directory, id }
  })
  const prompt = {
    ready: () => promptStore.ready(promptScope()),
    current: () => promptStore.current(promptScope()),
    cursor: () => promptStore.cursor(promptScope()),
    dirty: () => promptStore.dirty(promptScope()),
    context: {
      items: () => promptStore.context.items(promptScope()),
      add: (item: ContextItem) => promptStore.context.add(item, promptScope()),
      remove: (key: string) => promptStore.context.remove(key, promptScope()),
      replaceComments: (items: FileContextItem[]) => promptStore.context.replaceComments(items, promptScope()),
    },
    set: (value: Prompt, cursorPosition?: number) => promptStore.set(value, cursorPosition, promptScope()),
    reset: () => promptStore.reset(promptScope()),
  }
  ...
  const working = createMemo(() => sync.data.session_working(sessionID() ?? ""))
  ...
  const tip = () => {
    if (readOnly()) return language.t("prompt.action.readOnly")
    if (stopping()) {
      return (
            ...
            <div class="flex items-center gap-1 pointer-events-auto">
              <Tooltip placement="top" inactive={!working() && blank()} value={tip()}>
                <IconButton
                  ...
                  type="submit"
                  disabled={readOnly() || (!working() && blank())}
