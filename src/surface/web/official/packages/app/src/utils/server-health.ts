...
export type ServerHealth = { healthy: boolean; version?: string; authRole?: "read" | "write" }
  ...
  const attempt = (count: number): Promise<ServerHealth> =>
    createSdkForServer({
      ...
      .then((x) => {
        ...
        const role = x.response.headers.get("x-auth-role")
        return {
          healthy: x.data?.healthy === true,
          version: x.data?.version,
          authRole: role === "read" || role === "write" ? role : undefined,
        }
      })
