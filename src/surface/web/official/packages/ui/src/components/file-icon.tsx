...
const toOpenVariant = (icon: IconName | unknown): IconName => {
  if (typeof icon !== "string") return ICON_MAPS.defaults.folderOpen
  if (!icon.startsWith("Folder")) return icon as IconName
...
const basenameOf = (p: unknown) =>
  (typeof p === "string" ? p : "").split("\\").join("/").split("/").filter(Boolean).pop() ?? ""
...
export function chooseIconName(path: unknown, type: "directory" | "file", expanded: boolean): IconName {
