...
function mkUser(text: string, extra: Part[] = [], sessionID = SESSION_ID): { message: UserMessage; parts: Part[] } {
  ...
  return {
    ...
    parts: [
      { id: uid(), sessionID, messageID: id, type: "text", text, time: { start: Date.now() } } as TextPart,
...
function textPart(text: string, sessionID = SESSION_ID, messageID = ""): TextPart {
  return { id: uid(), sessionID, messageID, type: "text", text, time: { start: Date.now() } } as TextPart
}

function reasoningPart(text: string, sessionID = SESSION_ID, messageID = ""): ReasoningPart {
  return {
    id: uid(),
    sessionID,
    messageID,
...
function Playground() {
  ...
  const appendParts = (parts: Part[]) => {
    ...
    setState(
      produce((draft) => {
        ...
        draft.parts[id] = [
          ...existing,
          ...parts.map((part) => ({ ...part, sessionID: message?.sessionID ?? session().id, messageID: id })),
        ]
      }),
    )
  }
  ...
  const addFullTurn = (userText: string, parts: Part[]) => {
    ...
    setState(
      produce((draft) => {
        ...
        draft.messages.push(asst)
        draft.parts[user.message.id] = user.parts
        draft.parts[asst.id] = parts.map((part) => ({ ...part, sessionID: asst.sessionID, messageID: asst.id }))
      }),
    )
  }

  const addToolSequenceTurn = () => {
    addFullTurn("Run a command and delegate follow-up", [
      toolPart(TOOL_SAMPLES.bash),
      toolPart(TOOL_SAMPLES.task),
      textPart("After running active tools, here's what I found:\n\n" + LOREM[2]),
    ])
  }
  ...
  const addKitchenSink = () => {
    ...
    appendParts([toolPart(TOOL_SAMPLES.task), toolPart(TOOL_SAMPLES.bash), textPart(MARKDOWN_SAMPLES.blockquote)])
    addToolSequenceTurn()
    ...
    setState(
      produce((draft) => {
        ...
        if (msg) {
          ...
          msg.error = { name: "MessageAbortedError", data: { message: "Interrupted" } }
          return
        }

        const asst = mkAssistant(user.id, session().id)
        asst.time = { created: now, completed: now }
        asst.error = { name: "MessageAbortedError", data: { message: "Interrupted" } }
        draft.messages.push(asst)
        draft.parts[asst.id] = []
      }),
    )
        ...
        <div style={{ "border-bottom": "1px solid var(--border-weak-base)" }}>
          ...
          <Show when={panels.generators}>
            <div style={{ padding: "0 12px 12px", display: "flex", "flex-direction": "column", gap: "6px" }}>
              ...
              <div style={{ display: "flex", "flex-wrap": "wrap", gap: "4px" }}>
                <button style={btnStyle} onClick={addToolSequenceTurn}>
                  tool sequence
                </button>
