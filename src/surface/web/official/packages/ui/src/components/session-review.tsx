...
type ReviewDiff = RawReviewDiff & { file: string }
...
export const SessionReview = (props: SessionReviewProps) => {
  ...
  const selectionPreview = (diff: ViewDiff | undefined, range: SelectedLineRange) => {
              ...
              <Accordion multiple value={open()} onChange={handleChange}>
                <For each={items()}>
                  {(diff) => {
                    ...
                    const viewDiff = createMemo<ViewDiff | undefined>(() => {
                      if (!expanded()) return
                      if (tooLarge()) return
                    ...
                    const commentsUi = createLineCommentController<SessionReviewComment>({
                      ...
                      onSubmit: ({ comment, selection }) => {
                        props.onLineComment?.({
                          ...
                          selection,
                          comment,
                          preview: selectionPreview(viewDiff(), selection),
                        })
                      },
                          ...
                          >
                            <Show when={expanded()}>
                              <Switch>
                                ...
                                <Match when={true}>
                                  <Show when={viewDiff()} keyed>
                                    {(renderedDiff) => (
                                      <Dynamic
                                        component={fileComponent}
                                        mode="diff"
                                        fileDiff={renderedDiff.fileDiff}
