...
import { installPlugin, patchPluginConfig, readPluginManifest } from "@/init/plugin/install"
import { resolvePluginTarget } from "@/init/npm/plugin-shared"
  ...
  return async (ctx: PlugCtx) => {
    ...
    if (!manifest.ok) {
      ...
      if (manifest.code === "manifest_no_targets") {
        ...
        dep.log.info(
          'Expected one of: exports["@/surface/cli/cmd/tui"], exports["./server"], package.json main for server, or package.json["oc-themes"] for tui themes.',
        )
        return false
      }
