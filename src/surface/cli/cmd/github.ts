      ...
      async fn() {
        {
          ...
          async function getAppInfo() {
            ...
            // Get repo info
            const info = (
              await Process.text(["git", "remote", "get-url", "origin"], { cwd: Instance.worktree, nothrow: true })
            ).text.trim()
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const runGit = async (args: string[]) => {
        const out = await Process.run(["git", ...args], { cwd: Instance.worktree, nothrow: true })
        return {
          exitCode: out.code,
      ...
      const gitStatus = (args: string[]) => runGit(args)
      ...
      async function pushToNewBranch(summary: string, branch: string, commit: boolean, isSchedule: boolean) {
        ...
        if (commit) {
          await gitRun(["add", "@/surface/cli/cmd"])
          if (isSchedule) {
