...
export function formatTranscript(
  ...
  // gap-25-followup-1: thread Locale.resolved() so non-en users
  // see their LANG/LC_* configured month names instead of English
  // (Bun on macOS treats `undefined` as system default and ignores
  // POSIX env vars). Full datetime format is preserved — this is
  // metadata, not a chat-style timestamp where briefTimestamp would
  // apply.
  const lang = Locale.resolved()
  transcript += `**Created:** ${new Date(session.time.created).toLocaleString(lang)}\n`
  transcript += `**Updated:** ${new Date(session.time.updated).toLocaleString(lang)}\n\n`
  transcript += `---\n\n`
