...
import { errorData, errorMessage } from "@/foundation/util/error"
...
import {
  ...
  type PluginPackage,
  type PluginSource,
} from "@/init/npm/plugin-shared"
import { PluginLoader } from "@/init/plugin/loader"
import { PluginMeta } from "@/init/plugin/meta"
import { installPlugin as installModulePlugin, patchPluginConfig, readPluginManifest } from "@/init/plugin/install"
import { hasTheme, upsertTheme } from "@/surface/cli/cmd/tui/context/theme"
...
import { INTERNAL_TUI_PLUGINS, type InternalTuiPlugin } from "@/surface/cli/cmd/tui/plugin/internal"
import { setupSlots, Slot as View } from "@/surface/cli/cmd/tui/plugin/slots"
import type { HostPluginApi, HostSlots } from "@/surface/cli/cmd/tui/plugin/slots"
...
function createPluginScope(load: PluginLoad, id: string) {
  // gap-51-followup-2: createAbortController bumps the signal's
  // maxListeners to 50. The ctrl.signal is exposed to the plugin
  // via lifecycle.signal, and plugins commonly attach abort
  // listeners to react to disposal — bus subscriptions, fetch
  // requests, animation frames, etc. With a busy plugin
  // attaching 10+ listeners, the default Node cap (10) trips
  // MaxListenersExceededWarning. createAbortController solves
  // this with a 2-line change.
