...
// gap-25-followup-2: Locale.resolved() converts POSIX env vars to a
// BCP 47 tag so non-en users see locale-correct thousands separators
// on macOS Bun (V8 ignores POSIX env in raw `toLocaleString()`).
...
export function displayStats(stats: SessionStats, toolLimit?: number, modelLimit?: number) {
  ...
  console.log("├────────────────────────────────────────────────────────┤")
  console.log(renderRow("Sessions", stats.totalSessions.toLocaleString(Locale.resolved())))
  console.log(renderRow("Messages", stats.totalMessages.toLocaleString(Locale.resolved())))
  ...
  console.log(renderRow("Avg Tokens/Session", Locale.number(Math.round(tokensPerSession))))
  const medianTokensPerSession = isNaN(stats.medianTokensPerSession) ? 0 : stats.medianTokensPerSession
  console.log(renderRow("Median Tokens/Session", Locale.number(Math.round(medianTokensPerSession))))
  console.log(renderRow("Input", Locale.number(stats.totalTokens.input)))
  console.log(renderRow("Output", Locale.number(stats.totalTokens.output)))
  console.log(renderRow("Cache Read", Locale.number(stats.totalTokens.cache.read)))
  console.log(renderRow("Cache Write", Locale.number(stats.totalTokens.cache.write)))
  console.log("└────────────────────────────────────────────────────────┘")
  console.log()
  ...
  if (modelLimit !== undefined && Object.keys(stats.modelUsage).length > 0) {
    ...
    for (const [model, usage] of modelsToDisplay) {
      ...
      console.log(renderRow("  Messages", usage.messages.toLocaleString(Locale.resolved())))
      console.log(renderRow("  Input Tokens", Locale.number(usage.tokens.input)))
      console.log(renderRow("  Output Tokens", Locale.number(usage.tokens.output)))
      console.log(renderRow("  Cache Read", Locale.number(usage.tokens.cache.read)))
      console.log(renderRow("  Cache Write", Locale.number(usage.tokens.cache.write)))
  ...
  if (Object.keys(stats.toolUsage).length > 0) {
    ...
    for (const [tool, count] of toolsToDisplay) {
      ...
      const truncatedTool = tool.length > maxToolLength ? tool.substring(0, maxToolLength - 2) + "@/surface/cli" : tool
