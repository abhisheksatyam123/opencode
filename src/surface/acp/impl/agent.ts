...
import { ACPSessionManager } from "@/surface/acp/impl/session"
import type { ACPConfig } from "@/surface/acp/contract/types"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
          ...
          if (part.type === "tool") {
            ...
            switch (part.state.status) {
              ...
              case "running":
                ...
                if (output) {
                  const hash = Hash.content(output)
                  if (part.tool === "bash") {
    ...
    private async loadSessionMode(params: LoadSessionRequest) {
      ...
      if (!names.has("loop"))
        availableCommands.push({
          name: "loop",
          description: "run a prompt repeatedly in loop mode until all todos are done",
        })
  ...
  function toToolKind(toolName: string): ToolKind {
    return toolName.toLocaleLowerCase() === "bash" ? "execute" : "other"
  }

  function toLocations(_toolName: string, _input: Record<string, any>): { path: string }[] {
    return []
  }
  ...
  function parseModelSelection(
    ...
    // Try to extract a trailing variant segment from modelID.
