...
import { EventSequenceTable, EventTable } from "@/surface/sync/impl/event.sql"
import { EventID } from "@/surface/sync/contract/schema"
...
export namespace SyncEvent {
  ...
  export function reset() {
    frozen = false
    // Preserve installed projectors unless caller re-inits explicitly.
    // Tests/preload install projectors once process-wide; clearing here creates
    // cross-file order dependence and spurious "No projectors available" failures.
  ...
  function process<Def extends Definition>(def: Def, event: Event<Def>, options: { publish: boolean }) {
    if (projectors == null) {
      // Build-time constraint: `bun build --compile` forbids `require()` calls
      // whose transitive deps contain top-level await (src/global/index.ts has
      // TLA via xdg dir setup). The previous lazy-require fallback triggered
      // this. Callers must initialize projectors via the normal preload chain
      // before reaching sync code (see server/instance.ts, session/preload).
  ...
  // Backlog:
  // - Support applying multiple events at one time (one transaction, validate all sequence IDs)
  // - Apply zod validation to event shape when loading events from DB
        ...
        registry
          ...
          .map(([type, def]) => {
            return z
              ...
              .meta({
                ref: "SyncEvent" + "@/surface/sync/impl" + def.type,
              })
          })
