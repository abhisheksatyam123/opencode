...
export * from "@/filesystem/file/contract/port"
...
export namespace File {
  ...
  const hidden = (item: string) => {
    ...
    return normalized.split("/").some((part) => part.startsWith("@/filesystem/file") && part.length > 1)
  }
    ...
    Effect.gen(function* () {
      ...
      const scan = Effect.fn("File.scan")(function* () {
        ...
        if (isGlobalHome) {
          ...
          const shouldIgnoreName = (name: string) => name.startsWith("@/filesystem/file") || protectedNames.has(name)
          const shouldIgnoreNested = (name: string) => name.startsWith("@/filesystem/file") || ignoreNested.has(name)
      ...
      const status = Effect.fn("File.status")(function* () {
        ...
        return yield* Effect.promise(async () => {
          const diffOutput = (
            await Process.text(
              ["git", "-c", "core.fsmonitor=false", "-c", "core.quotepath=false", "diff", "--numstat", "HEAD"],
              {
      ...
      const read = Effect.fn("File.read")(function* (file: string) {
        ...
        if (Instance.project.vcs === "git") {
          return yield* Effect.promise(async (): Promise<File.Content> => {
            let diff = (
              await Process.text(["git", "-c", "core.fsmonitor=false", "diff", "--", file], {
                cwd: Instance.directory,
                nothrow: true,
              })
            ).text
            if (!diff.trim()) {
              diff = (
                await Process.text(["git", "-c", "core.fsmonitor=false", "diff", "--staged", "--", file], {
                  cwd: Instance.directory,
                  nothrow: true,
                })
              ).text
            ...
            if (diff.trim()) {
              const original = (
                await Process.text(["git", "show", `HEAD:${file}`], {
                  cwd: Instance.directory,
                  nothrow: true,
                })
              ).text
      ...
      const search = Effect.fn("File.search")(function* (input: {
        ...
        const preferHidden = query.startsWith("@/filesystem/file") || query.includes("/.")
