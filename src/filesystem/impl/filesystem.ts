...
export namespace AppFileSystem {
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // gap-bom-1-followup: PowerShell 5.x writes UTF-8 with BOM by default.
      // Mirror the strip in `util/filesystem.ts:readJson` so the Effect-based
      // AppFileSystem.readJson handles PS-written config files cleanly too.
      // (Both `util/filesystem` and `filesystem/index` are independent JSON
      // entry points — both must strip for consistent Windows UX.)
