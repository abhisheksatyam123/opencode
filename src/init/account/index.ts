...
import { withTransientReadRetry } from "@/foundation/util/effect-http-client"
import { AccountRepo, type AccountRow } from "@/init/account/repo"
import { registerConfigAccountBridge } from "@/config/account-bridge"
import {
  type AccountError,
  AccessToken,
  AccountID,
  ...
  export const layer: Layer.Layer<Service, never, AccountRepo | HttpClient.HttpClient> = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const poll = Effect.fn("Account.poll")(function* (input: Login) {
        ...
        // Follow-up: when there are multiple orgs, let the user choose
...
registerConfigAccountBridge({
  activeOrg: () => Account.activeOrg(),
  config: async (accountID, orgID) =>
    Account.runPromise((service) =>
      service
        .config(AccountID.make(String(accountID)), OrgID.make(String(orgID)))
        .pipe(Effect.map(Option.getOrUndefined)),
    ),
  token: (accountID) => Account.token(AccountID.make(String(accountID))),
})
