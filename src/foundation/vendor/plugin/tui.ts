...
export type TuiState = {
  ...
  session: {
    ...
    workspace_todos: () => ReadonlyArray<Todo>
    attached_todo_ids: (sessionID: string) => ReadonlyArray<string>
    attached_todo_labels: (sessionID: string) => Record<string, string>
  }
...
// Standalone type instead of Pick<Todo, ...> because the SDK Todo type
// (auto-generated from the OpenAPI spec) does not yet expose `agent` or
// `comments`. When the SDK is regenerated with those fields, switch back
// to `Pick<Todo, "content" | "status" | "priority" | "agent" | "comments">`.
