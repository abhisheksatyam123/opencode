...
export namespace Flag {
  ...
  // Centralization (flag-followup-1): the share subsystem previously
  // read this directly via inline `process.env[...] === "true" || === "1"`
  // — same shape as our `truthy()` helper, now routed through Flag for
  // discoverability.
  export const OPENCODE_DISABLE_SHARE = truthy("OPENCODE_DISABLE_SHARE")
  // Centralization (flag-followup-1): the TUI route bootstrap previously
  // read this directly to seed its initial Route store from a JSON-encoded
  // env var. Stored as a raw string — JSON.parse stays in the consumer.
  export const OPENCODE_ROUTE = process.env["OPENCODE_ROUTE"]
  ...
  // gap-22-followup-3 + gap-23-followup-3: same opt-out for MCP
  // tool results. MCP servers are external (third-party) processes
  // that produce arbitrary text content — same threat model as
  // webfetch and websearch. Default OFF (sanitization on). Set
  // to "true"/"1" if you trust every MCP server you've configured
  // and need raw output for binary diff/audit work.
  export const OPENCODE_DISABLE_MCP_SANITIZE = truthy("OPENCODE_DISABLE_MCP_SANITIZE")
  ...
  export const OPENCODE_SERVER_READ_PASSWORD = process.env["OPENCODE_SERVER_READ_PASSWORD"]
  export const OPENCODE_SERVER_USERNAME = process.env["OPENCODE_SERVER_USERNAME"]
  export const OPENCODE_WEB = truthy("OPENCODE_WEB")
  export const OPENCODE_PERMISSION_TIMEOUT_SECONDS = number("OPENCODE_PERMISSION_TIMEOUT_SECONDS")
