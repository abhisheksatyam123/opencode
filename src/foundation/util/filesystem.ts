...
export namespace Filesystem {
  ...
  // gap-bom-1: PowerShell 5.x writes UTF-8 with BOM by default
  // (Out-File, Set-Content). Without stripping it, JSON.parse fails
  // on the leading U+FEFF byte ("Unexpected token in JSON at position 0").
  // Strip on read so opencode reads PS-written config files cleanly —
  // the BOM is metadata, not content. Same shape as Claude Code's
  // `utils/jsonRead.ts:stripBOM`. Stripped from JSON only because
  // text-byte-precise consumers (line counting, character-precise
  // operations) might depend on the BOM in `readText` output.
  const UTF8_BOM = "\uFEFF"
  ...
  export async function writeStream(
    ...
    const nodeStream = stream instanceof ReadableStream ? Readable.fromWeb(stream as any) : stream // as any: Node's Readable.fromWeb expects NodeJS.ReadableStream; web ReadableStream is compatible at runtime
