// util/secret-scan.ts
//
// Client-side secret scanner (parity gap-22).
//
// PROVENANCE: cp'd from
// `instructkr-claude-code/src/services/teamMemorySync/secretScanner.ts`
// then adapted to opencode:
//
//   * dropped `import { capitalize }` — inlined the helper at the
//     bottom of this file (it's three lines).
//   * wrapped the helpers in a `SecretScan` namespace following
//     opencode convention (`Hash`, `Binary`, `Token`, …).
//   * the SECRET_RULES table is otherwise UNCHANGED — every rule is
//     curated from the gitleaks config and bumping versions
//     manually here would create silent drift from upstream.
//
// PURPOSE: opencode pipes a lot of arbitrary content through the
// model — bash output, file reads, web fetches, MCP tool results.
// Without a scanner, a stray `AKIA…` or `sk-ant-api03-…` in any of
// these flows ends up in the conversation history (and the prompt
// cache, and the compaction summary, and the notes vault if the
// model decides to log it). The scanner is a building block for:
//
//   * `tool.execute.after` lifecycle hook (gap-1) that scrubs tool
//     outputs before they reach the model.
//   * `write` pre-write check that aborts on a positive match.
//   * `webfetch` post-fetch check that warns the user.
//   * `bash` output sanitizer (paired with `redact`).
//
// Uses a curated subset of high-confidence rules from gitleaks
// (https://github.com/gitleaks/gitleaks, MIT license). Only rules
// with distinctive prefixes that have near-zero false-positive
// rates are included. Generic keyword-context rules are omitted.
// Rule IDs and regexes sourced directly from the public gitleaks
// config:
// https://github.com/gitleaks/gitleaks/blob/master/config/gitleaks.toml
//
// JS REGEX NOTES:
//   * gitleaks uses Go regex; inline `(?i)` and mode groups
//     `(?-i:...)` are not portable to JS. Affected rules are
//     rewritten with explicit character classes
//     (`[a-zA-Z0-9]` instead of `(?i)[a-z0-9]`).
//   * Trailing boundary alternations like
//     `(?:[\x60'"\s;]|\\[nr]|$)` from Go regex are kept (JS `$`
//     matches end-of-string in default mode).
...
export function redactSecrets(content: string): string {
  ...
  // Inlined from Claude's `utils/stringUtils.ts` to avoid pulling the
  // whole stringUtils dependency for one tiny helper.
  function capitalize(s: string): string {
    if (s.length === 0) return s
    return s.charAt(0).toUpperCase() + s.slice(1)
  }
} // end namespace SecretScan
