...
import { InstanceContextStorage, type InstanceContext } from "./instance-context"
...
export namespace InstanceState {
  export const bind = <F extends (...args: any[]) => any>(fn: F): F => {
    try {
      return InstanceContextStorage.bind(fn)
    } catch (err) {
    ...
    if (!ctx) return fn
    return ((...args: any[]) => InstanceContextStorage.restore(ctx, () => fn(...args))) as F
  }

  export const context = Effect.gen(function* () {
    return (yield* InstanceRef) ?? InstanceContextStorage.current
  })
   ...
   */
  export const withALS = <T>(fn: () => T) => Effect.map(context, (ctx) => InstanceContextStorage.restore(ctx, fn))
}
