...
// gap-error-followup-4: util/error.ts:toError replaces the inline
// `err instanceof Error ? err : new Error(String(err))` helper that
// previously lived here. Centralized so consumers across mcp/index.ts
// and other catch sites get the same wrapping semantics, including
// the errorMessage() fallback for plain object errors.
...
const rt = lazy(async () => {
  ...
  const { makeRuntime } = await import("@/foundation/effect/run-service")
