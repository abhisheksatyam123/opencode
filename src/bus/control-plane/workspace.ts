...
import { WorkspaceTable } from "@/bus/control-plane/workspace.sql"
import { getAdaptor } from "@/bus/control-plane/adaptors"
import { WorkspaceInfo } from "@/bus/control-plane/types"
import { WorkspaceID } from "@/bus/control-plane/schema"
import { parseSSE } from "@/bus/control-plane/sse"
...
export namespace Workspace {
  ...
  export function startSyncing(project: Project.Info) {
    // gap-51-followup-1: createAbortController bumps the signal's
    // maxListeners to 50 so attaching one abort listener per
    // workspaceEventLoop call (one per workspace) doesn't trip
    // MaxListenersExceededWarning when many workspaces are
    // configured. The default Node limit is 10.
    const stop = createAbortController()
